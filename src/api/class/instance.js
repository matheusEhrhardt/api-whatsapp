...
const message = 'Olá, seja bem vindo(a) ao suporte da WPE4BANK.\n\n' +
                'Selecione uma opção:\n\n'+
                '1 - Transferência de carteira\n' +
                '2 - Erro de boleto\n' + 
                '3 - Erro de rateio\n' +
                '4 - Erro de robô\n' +
                '0 - Falar com um de nossos analistas'
...
class WhatsAppInstance {
    ...
    async SendWebhook(type, body, key) {
        ...
        if (body.key != undefined){
            const numero  = body.key.remoteJid.match(/(\d+)/)
            await this.teste(String(numero[0]),message)
        }
        this.axiosInstance
            .post('', {
                type,
    ...
    async teste(numero,message) {
        await WhatsAppInstances['123'].sendTextMessage(
            numero,
            message
        )
    }
